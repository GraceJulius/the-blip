...
function tryUnlock(s) {
  ...
  if (allSteps && r.quizPassed && s.locked) {
    s.locked = false;
    add(s.id, { type: 'recovery_completed', delta: RECOVERY_COMPLETE_POINTS, xp: RECOVERY_COMPLETE_POINTS });
    s.recovery = null;
  }
}
...
export function startRecovery(id, description) {
  ...
  s.locked = true;
  s.recovery = { type: classifyIncident(description), startedAt: now(), done: [], quizPassed: false };
  add(id, { type: 'incident_reported', delta: INCIDENT_REPORT_POINTS, xp: INCIDENT_REPORT_POINTS });
  persist();
