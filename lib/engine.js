...
// Bank sandbox alerts: stored in the ledger with no points, so the student's Home screen can show them.
export function pushBankAlert(studentId, alert) {
  ensure(studentId);
  add(studentId, { type: 'bank_alert', delta: 0, xp: 0, alert: { severity: alert.severity, title: alert.title, message: alert.message } });
}
...
export function getState(id) {
  ...
  return {
    ...
    recent: db().ledger.filter((l) => l.studentId === id && l.type !== 'bank_alert').slice(-8).reverse(),
    alerts: db().ledger.filter((l) => l.studentId === id && l.type === 'bank_alert').slice(-3).reverse().map((l) => ({ ...l.alert, ts: l.ts })),
  };
}
